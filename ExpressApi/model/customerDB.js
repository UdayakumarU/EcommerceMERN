...
customerModel.insertScript = () => {
    return collection.getCollection("Products").then((model) => {
        return model.deleteMany().then((data) => {
            return model.insertMany(initialData).then((response) => {
...
customerModel.getPrductById = productId => {
    return collection.getCollection("Products").then((model) => {
        return model.findOne({ "productId" : productId}).then((data) => {
            if(data) return data;
            else{
                let err = new Error("Sorry! you don't have this product")
                err.status = 404;
                throw err;
            }
        }) 
